use crate::dev_print;
use reqwest::Url;

use std::{
    fs,
    path::{Path, PathBuf},
};

use anyhow::{bail, Context, Result};
use kuchiki::traits::*;
use walkdir::WalkDir;

use image::{
    GenericImageView,
//    ImageFormat,
    ImageReader,
    imageops::FilterType,
};

pub fn find_dist_html_files(path: &Path) -> Result<Vec<PathBuf>> {
    if !path.is_dir() {
        bail!(
            "Dist site directory does not exist: {}",
            path.display()
        );
    }

    let mut html_files = Vec::new();

    for entry in WalkDir::new(&path).sort_by_file_name() {
        let entry = entry.with_context(|| {
            format!(
                "Failed while walking {}",
                path.display()
            )
        })?;

        if !entry.file_type().is_file() {
            continue;
        }

        let file_path = entry.path();

        if file_path.extension().is_some_and(|ext| ext == "html") {
            html_files.push(file_path.to_path_buf());
        }
    }

    Ok(html_files)
}

pub fn optimize_html_file(
    dist_site_dir: &Path,
    html_path:     &Path,
) -> Result<()> {
    dev_print!("Optimizing {}", &html_path.display());

    let html = fs::read_to_string(html_path).with_context(|| {
        format!(
            "Failed to read HTML file: {}",
            html_path.display()
        )
    })?;

    let document = kuchiki::parse_html().one(html);

    optimize_img_tags(dist_site_dir, html_path, &document).with_context(|| {
        format!(
            "Failed to optimize image tags in: {}",
            html_path.display()
        )
    })?;

    let mut output = Vec::new();

    document.serialize(&mut output).with_context(|| {
        format!(
            "Failed to serialize optimized HTML file: {}",
            html_path.display()
        )
    })?;

    fs::write(html_path, output).with_context(|| {
        format!(
            "Failed to write optimized HTML file: {}",
            html_path.display()
        )
    })?;

    Ok(())
}

fn optimize_img_tags(
    dist_site_dir: &Path,
    html_path:     &Path,
    document:      &kuchiki::NodeRef,
) -> Result<()> {
    let img_tags = document
        .select("img")
        .map_err(|_| anyhow::anyhow!("Failed to select img tags"))?;

    for img in img_tags {
        //dev_print!(" processing img tag: {}", &img);
        optimize_img_tag(dist_site_dir, html_path, img.as_node())?;
    }

    Ok(())
}

fn optimize_img_tag(
    dist_site_dir: &Path,
    html_path:     &Path,
    img:           &kuchiki::NodeRef,
) -> Result<()> {
    let element = img
        .as_element()
        .context("img selector returned a non-element node")?;

    let (src, existing_srcset) = {
        let attrs = element.attributes.borrow();

        let Some(src) = attrs.get("src") else {
            return Ok(());
        };

        let existing_srcset = attrs
            .get("srcset")
            .map(|s| s.trim().to_string());

        (src.to_string(), existing_srcset)
    };

    if existing_srcset.is_some_and(|s| !s.is_empty()) {
        return Ok(());
    }

    let Some(image_path) = image_path_for_src(
        dist_site_dir,
        html_path,
        &src,
    )? else {
        return Ok(());
    };

    if !is_optimizable_image(&image_path) {
        return Ok(());
    }

    if !image_path.is_file() {
        bail!(
            "Image referenced by {} does not exist on disk: {} -> {}",
            html_path.display(),
            src,
            image_path.display()
        );
    }

    let reader = ImageReader::open(&image_path).with_context(|| {
        format!("Failed to open image: {}", image_path.display())
    })?;

    let format = reader.format().with_context(|| {
        format!("Failed to detect image format: {}", image_path.display())
    })?;

    let image = reader.decode().with_context(|| {
        format!("Failed to decode image: {}", image_path.display())
    })?;

    let (original_width, original_height) = image.dimensions();

    let widths = responsive_widths(original_width);

    if widths.is_empty() {
        return Ok(());
    }

    let mut srcset_entries = Vec::new();

    // Include original as the largest candidate.
    srcset_entries.push(format!("{} {}w", clean_src_for_srcset(&src), original_width));

    for width in widths {
        let height = scaled_height(original_width, original_height, width);
        let variant_path = variant_path_for_width(&image_path, width)?;

        if !variant_path.exists() {
            let resized = image.resize_exact(
                width,
                height,
                FilterType::Lanczos3,
            );

            resized
                .save_with_format(&variant_path, format)
                .with_context(|| {
                    format!(
                        "Failed to write resized image variant: {}",
                        variant_path.display()
                    )
                })?;
        }

        let variant_src = variant_src_for_width(&src, width)?;
        srcset_entries.push(format!("{} {}w", variant_src, width));
    }

    let mut attrs = element.attributes.borrow_mut();

    attrs.insert("srcset", srcset_entries.join(", "));
    //TODO: Rules should determine laziness
    //attrs.insert("loading", "lazy".to_string());
    attrs.insert("decoding", "async".to_string());

    Ok(())
}

fn is_non_local_reference(source: &str) -> bool {
    let source = source.trim();

    source.starts_with('#')
        || source.starts_with("//")
        || Url::parse(source).is_ok()
}

fn image_path_for_src(
    dist_site_dir: &Path,
    html_path: &Path,
    src: &str,
) -> Result<Option<PathBuf>> {
    let src = src.trim();

    if src.is_empty() || is_non_local_reference(src) {
        return Ok(None);
    }

    let clean_src = src
        .split('#')
        .next()
        .unwrap_or(src)
        .split('?')
        .next()
        .unwrap_or(src);

    if clean_src.is_empty() {
        return Ok(None);
    }

    let base = if clean_src.starts_with('/') {
        dist_site_dir
    } else {
        html_path.parent().with_context(|| {
            format!(
                "HTML file has no parent directory: {}",
                html_path.display()
            )
        })?
    };

    let image_path = crate::utils::resolve_within(
        dist_site_dir,
        base,
        clean_src,
    )?;

    Ok(Some(image_path))
}

fn is_optimizable_image(path: &Path) -> bool {
    path.extension()
        .and_then(|ext| ext.to_str())
        .map(|ext| {
            matches!(
                ext.to_ascii_lowercase().as_str(),
                "jpg" | "jpeg" | "png" | "webp"
            )
        })
        .unwrap_or(false)
}

fn responsive_widths(original_width: u32) -> Vec<u32> {
    const MIN_WIDTH: u32 = 100;

    let mut widths = Vec::new();
    let mut width = ((original_width as f32) * 0.9).round() as u32;

    while width >= MIN_WIDTH {
        widths.push(width);
        width = ((width as f32) * 0.8).round() as u32;
    }

    widths
}

fn scaled_height(
    original_width:  u32,
    original_height: u32,
    target_width:    u32,
) -> u32 {
    ((original_height as f64) * (target_width as f64 / original_width as f64))
        .round()
        .max(1.0) as u32
}

fn variant_path_for_width(
    image_path: &Path,
    width:      u32,
) -> Result<PathBuf> {
    let parent = image_path.parent().with_context(|| {
        format!(
            "Image path has no parent directory: {}",
            image_path.display()
        )
    })?;

    let stem = image_path
        .file_stem()
        .and_then(|s| s.to_str())
        .with_context(|| {
            format!(
                "Image path has invalid file stem: {}",
                image_path.display()
            )
        })?;

    let ext = image_path
        .extension()
        .and_then(|s| s.to_str())
        .with_context(|| {
            format!(
                "Image path has invalid extension: {}",
                image_path.display()
            )
        })?;

    Ok(parent.join(format!("{}-{}w.{}", stem, width, ext)))
}

fn variant_src_for_width(
    src:   &str,
    width: u32,
) -> Result<String> {
    let clean_src = clean_src_for_srcset(src);

    let path = Path::new(&clean_src);

    let parent = path.parent();
    let stem = path
        .file_stem()
        .and_then(|s| s.to_str())
        .with_context(|| {
            format!("Image src has invalid file stem: {}", src)
        })?;

    let ext = path
        .extension()
        .and_then(|s| s.to_str())
        .with_context(|| {
            format!("Image src has invalid extension: {}", src)
        })?;

    let filename = format!("{}-{}w.{}", stem, width, ext);

    match parent {
        Some(parent) if parent != Path::new("") => {
            Ok(parent.join(filename).to_string_lossy().to_string())
        }
        _ => Ok(filename),
    }
}

fn clean_src_for_srcset(src: &str) -> String {
    src.split('#')
        .next()
        .unwrap_or(src)
        .split('?')
        .next()
        .unwrap_or(src)
        .to_string()
}
